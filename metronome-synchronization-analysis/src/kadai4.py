...
cap = cv2.VideoCapture('4623044_kadai4.MP4')
...
while(i):
    ...
    #HSV範囲
    lower_value = np.array([160, 80, 90])
    upper_value = np.array([179, 180, 190])
...
peaks, _ = find_peaks(angles, height=10)  # >10のピーク
frame_intervals = np.diff(peaks)          # 正のピークの間隔
avg_period_frames = np.mean(frame_intervals)
T= avg_period_frames / fps
BPM = 120 / T

print("="*40)
print("時系列解析結果：")
print(f"fps = {fps:.2f}")
print(f"average_peak_interval (frame) = {avg_period_frames:.2f}")
print(f"T = {T:.4f} [s]")
...
plt.plot([times[i] for i in peaks], [angles[i] for i in peaks], "rx")
plt.xlabel('Time (s)')
plt.ylabel('Angle (°)')
plt.grid(True)
plt.legend()
